    ...
    <section class="flex justify-center m-5">
        <div class="sm:mt-12 w-full">
            <div class="mx-auto max-w-screen-xl sm:px-6 lg:px-8">
                <form action="{{ route('verifyProfile', ['user' => Auth::user()->id]) }}" method="post" enctype="multipart/form-data"> @csrf
                    <div class="mb-6 text-center">
                        <label for="slug" class="mt-1 text-sm font-medium leading-relaxed text-indigo-600">Description</label>
                        <textarea name="description" maxlength="1000" id="message" rows="4" style="border: none" class="mt-1 bg-gray-50 text-gray-900 text-sm rounded-lg block w-full p-2.5 @if($user->dayVsNight == 1) bg-gray-900 dark:text-gray-400 @endif shadow-sm dark:placeholder-gray-400 " placeholder="Some description"></textarea>
                        <p class="mt-1 text-sm @if($user->dayVsNight == 1) text-gray-500 @else text-gray-500 @endif" id="favicon">max length 100 symbols</p>
                    </div>
                    <div class="mb-6 text-center">
                        <label for="description" class="mt-1 text-sm font-medium leading-relaxed text-indigo-600">Contacts</label>
                        <textarea name="contacts" maxlength="255" id="message" rows="4" style="border: none" class="mt-1 bg-gray-50 text-gray-900 text-sm rounded-lg block w-full p-2.5 @if($user->dayVsNight == 1) bg-gray-900 dark:text-gray-400 @endif shadow-sm dark:placeholder-gray-400 " placeholder="Contacts..."></textarea>
                    ...
                    <div class="mb-3 text-center">
                        <label for="description" class="mt-1 text-sm font-medium leading-relaxed text-indigo-600">Photo</label>
                        <input name="photo" class="mt-3 bg-gray-50 text-gray-900 text-sm rounded-lg block w-full @if($user->dayVsNight == 1) bg-gray-900 dark:text-gray-400 @endif shadow-sm dark:placeholder-gray-400" aria-describedby="avatar" id="avatar" type="file">
                        <p class="mt-1 text-sm @if($user->dayVsNight == 1) text-gray-500 @else text-gray-500 @endif" id="avatar">PNG, JPG, JPEG (MAX Size. 10mb).</p>
                    </div>
                    <div class="mt-6">
