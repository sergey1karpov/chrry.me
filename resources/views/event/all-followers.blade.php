    ...
    @foreach($countries as $country)
        <section class="content-block text-white @if($user->dayVsNight == 1) bg-black @endif">
            <div class="mx-auto max-w-screen-xl mt-4 sm:px-6 lg:px-8">
                <a href="{{ route('getAllEventCities', ['user' => $user->id, 'country' => $country->country_id]) }}" class="block w-full p-6 rounded-lg shadow @if($user->dayVsNight == 1) bg-[#0f0f0f] @else bg-gray-300 @endif">
                    <div class="flex flex-col items-center justify-center">
                        <h5 class="mb-2 text-4xl font-bold tracking-tight @if($user->dayVsNight == 1) text-white @else text-gray-800 @endif">{{$country->name}}</h5>
                        <dd class="@if($user->dayVsNight == 1) text-white @else text-gray-700 @endif">Всего подписчиков:</dd>
                        <dt class="text-3xl font-extrabold @if($user->dayVsNight == 1) text-white @else text-gray-800 @endif"><span class="text-blue-600 dark:text-blue-500">{{$country->countFollowers}}</span></dt>
                    </div>
                </a>
            </div>
        </section>
